// ==========================================
// EMOS 专属精装版反代 (完美聚合 + 奈飞海报 + 呼吸渐变 + 全端自适应)
// ==========================================

// ======== 基础配置区 ========
const PROXY_ID = "ePxxxxx";    // 你的 ID
const PROXY_NAME = "xxxxxx"; // 你的 称号 (已做安全编码处理)
const TARGET_HOST = "emos.best";  // 官方主服地址
const TMDB_API_KEY = "eyJhbGcxxxxxxxxx"; // 你的长令牌
// ==========================================
...
async function handleLandingPage(request, ctx) {
  ...
  <head>
      ...
      <style>
          ...
          /* 无痕呼吸渐变 LOGO，增加 padding-right 解决 s 被切边的问题 */
          .text-gradient-breathe {
              background: linear-gradient(135deg, #E50914, #9333ea);
              -webkit-background-clip: text;
              -webkit-text-fill-color: transparent;
              background-clip: text;
              animation: breathe-color 8s ease-in-out infinite alternate;
              padding-right: 0.1em; 
          }
  ...
  <!-- 外层容器修改：支持自适应高度，小屏幕可滚动，大屏幕居中 -->
  <body class="min-h-screen flex flex-col px-4 py-8 md:py-12">
      ...
      <!-- 内容区 my-auto 保证页面短时居中，超出时正常滚动 -->
      <div class="w-full max-w-5xl z-10 my-auto mx-auto flex flex-col">
          <header class="mb-10 md:mb-12 flex flex-col items-center text-center">
              <!-- 字体响应式：手机缩小到 6xl，平板以上 8xl -->
              <h1 class="text-6xl sm:text-7xl md:text-8xl font-black mb-1 md:mb-2 tracking-tighter text-gradient-breathe">emos</h1>
              <!-- 标语响应式 -->
              <h2 class="text-2xl sm:text-3xl md:text-5xl font-black mb-3 md:mb-4 tracking-wider">海量影视，无界畅享。</h2>
              <p class="text-base md:text-xl text-gray-300 font-light">私有云媒体智能代理节点已就绪。</p>
          </header>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              
              <!-- 智能测速卡片 -->
              <div class="nf-card flex flex-col justify-center items-center py-6 md:py-8">
                  <span class="text-gray-400 text-xs md:text-sm mb-2 uppercase tracking-widest border-b border-gray-700 pb-1">当前网络延迟</span>
                  <div class="flex items-baseline mt-2 mb-2">
                      <span class="text-5xl md:text-6xl font-bold ${pingColor}">${pingMs}</span>
                      <span class="text-lg md:text-xl text-gray-500 ml-1">ms</span>
                  </div>
                  <span class="text-[10px] md:text-xs font-bold px-3 py-1 rounded-full ${networkBadgeColor} tracking-widest mt-2 shadow-lg">
                      ${networkStatusText}
                  </span>
              </div>
              ...
              <div class="nf-card flex flex-col justify-center">
                  <h3 class="text-base md:text-lg font-bold mb-3 md:mb-4 border-b border-gray-700 pb-2 text-gray-200">连接详情</h3>
                  <div class="space-y-3 md:space-y-4 text-xs md:text-sm">
                      ...
                      <div class="flex justify-between items-center text-gray-400">
                          <span>网络归属</span>
                          <span class="text-white font-medium truncate max-w-[150px] text-right">${city}, ${country}</span>
                      </div>
                      <div class="flex flex-col mt-2">
                          <span class="text-gray-400 mb-1">访问 IP</span>
                          <span class="font-mono text-cyan-400 bg-black/50 p-2 rounded-lg border border-gray-800 text-center tracking-widest break-all">${clientIP}</span>
                      </div>
                  </div>
              </div>
              ...
              <!-- 系统状态卡片 -->
              <div class="nf-card flex flex-col justify-center">
                  <h3 class="text-base md:text-lg font-bold mb-3 md:mb-4 border-b border-gray-700 pb-2 text-gray-200">运行参数</h3>
                  <div class="space-y-3 md:space-y-4 text-xs md:text-sm mt-1">
                      ...
                      <div class="flex justify-between items-center text-gray-300">
                          <span>智能聚合</span>
                          <span class="text-cyan-400 border border-cyan-800 bg-cyan-900/30 px-2 py-0.5 rounded font-bold text-[10px] md:text-xs tracking-widest">ACTIVE</span>
                      </div>
                  </div>
              </div>
          </div>
          ...
          <!-- 浅绿色透明提示横幅 -->
          <div class="mt-6 md:mt-8 border border-green-500/30 bg-green-900/30 text-green-100 p-4 md:p-5 rounded-2xl flex items-start backdrop-blur-md shadow-lg">
              <svg class="w-5 h-5 md:w-6 md:h-6 mr-3 flex-shrink-0 text-green-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          ...
          <!-- 页脚 -->
          <footer class="mt-12 md:mt-16 text-center border-t border-gray-800 pt-6 md:pt-8 pb-4 flex flex-col items-center">
              
              <!-- 官方 Wiki 链接按钮 -->
              <a href="https://wiki.emos.best/" target="_blank" class="mb-5 md:mb-6 group flex items-center justify-center space-x-2 bg-gray-800/50 hover:bg-gray-700/80 border border-gray-700 hover:border-gray-500 transition-all duration-300 rounded-full px-4 py-2 md:px-5 backdrop-blur-sm cursor-pointer shadow-lg hover:shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                  <svg class="w-4 h-4 md:w-5 md:h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477-4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                  </svg>
                  <span class="text-xs md:text-sm font-bold text-gray-300 group-hover:text-white transition-colors tracking-widest">emos 官方 Wiki</span>
              </a>

              <p class="text-[10px] md:text-xs text-gray-600 mb-1 md:mb-2 tracking-widest uppercase">基于 Cloudflare 边缘计算强力驱动</p>
              <p class="text-[10px] md:text-xs text-gray-500 tracking-wider">Node Maintainer: <span class="text-gray-400 font-bold">${PROXY_NAME}</span> &copy; ${currentYear}</p>
          </footer>
      </div>
  </body>
  </html>
